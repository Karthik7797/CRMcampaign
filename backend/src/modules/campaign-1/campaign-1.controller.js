...
// PUBLIC endpoint — called from Campaign 1 Google Form (no auth needed)
export async function publicCreateCampaign1Lead(request, reply) {
  const {
  ...
  const lead = await db.campaign1Lead.create({
    data: {
      studentName,
      studentEmail,
      studentMobile,
...
export async function getCampaign1Leads(request, reply) {
  try {
    ...
    console.error('=== CAMPAIGN 1 LEADS ERROR ===')
    ...
    return reply.status(500).send({
      error: 'Failed to fetch campaign 1 leads',
...
export async function getCampaign1Lead(request, reply) {
  const lead = await db.campaign1Lead.findUnique({
  ...
  if (!lead) return reply.status(404).send({ error: 'Campaign 1 lead not found' })
...
export async function createCampaign1Lead(request, reply) {
  try {
    const lead = await db.campaign1Lead.create({
    ...
    request.log.error('Error creating campaign 1 lead:', error)
    return reply.status(500).send({
      error: 'Failed to create campaign 1 lead',
      message: error.message
    })
  }
}

export async function updateCampaign1Lead(request, reply) {
  // COUNSELLOR can only edit their own assigned leads
  if (request.user.role === 'COUNSELLOR') {
    const existing = await db.campaign1Lead.findUnique({ where: { id: request.params.id } })
    if (!existing) return reply.status(404).send({ error: 'Campaign 1 lead not found' })
  ...
  const lead = await db.campaign1Lead.update({
    where: { id: request.params.id },
    data: request.body,
  })

  if (request.body.status) {
    await db.campaign1LeadActivity.create({
      data: {
        type: 'STATUS_CHANGE',
...
export async function deleteCampaign1Lead(request, reply) {
  await db.campaign1Lead.delete({ where: { id: request.params.id } })
  return { success: true }
}

export async function assignCampaign1Lead(request, reply) {
...
export async function addCampaign1LeadNote(request, reply) {
  ...
  const note = await db.campaign1LeadNote.create({
    data: {
...
export async function getCampaign1LeadNotes(request, reply) {
  const { id } = request.params

  const notes = await db.campaign1LeadNote.findMany({
    where: { leadId: id },
    orderBy: { createdAt: 'desc' },
    include: {
      user: { select: { id: true, name: true, avatar: true } }
...
export async function moveCampaign1LeadStage(request, reply) {
...
// ── Tasks for Campaign 1 Leads ───────────────────────────────────────

export async function getCampaign1LeadTasks(request, reply) {
  const { id } = request.params

  const tasks = await db.campaign1LeadTask.findMany({
    where: { leadId: id },
...
export async function createCampaign1LeadTask(request, reply) {
  const { id } = request.params
  const { title, description, dueDate, priority } = request.body

  const task = await db.campaign1LeadTask.create({
    data: {
      title,
...
// ── Communications for Campaign 1 Leads ──────────────────────────────

export async function getCampaign1LeadComms(request, reply) {
  const { id } = request.params

  const comms = await db.campaign1LeadCommunication.findMany({
    where: { leadId: id },
    orderBy: { createdAt: 'desc' }
  })
...
export async function createCampaign1LeadComm(request, reply) {
  const { id } = request.params
  const { type, subject, content } = request.body

  const comm = await db.campaign1LeadCommunication.create({
    data: {
      type,
