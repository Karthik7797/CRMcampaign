// Campaign 3 — "Sai Story x Student ID Registration (UK & IRE)" lead detail.
import { useState, useEffect, useRef } from 'react'
import { useParams, useNavigate, useLocation } from 'react-router-dom'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { campaign3Api } from '../api/client'
import { ArrowLeft, User, Mail, Phone, Save, Edit2, Loader2, Send, MessageSquare, FileText, Calendar, ChevronLeft, ChevronRight } from 'lucide-react'
...
import {
  COUNTRY_OPTIONS, INTAKE_OPTIONS, QUALIFICATION_OPTIONS, DEGREE_OPTIONS,
  COMPLETION_YEAR_OPTIONS, PERCENTAGE_OPTIONS, MODE_OPTIONS,
} from './Campaign3Leads'

// Small helper: a labelled dropdown that shows a read-only value when not editing.
function SelectField({ id, label, value, options, isEditing, onChange, current }: {
  id: string; label: string; value: string; options: string[]; isEditing: boolean
  onChange: (v: string) => void; current: string
}) {
  return (
    <div className="space-y-1.5">
      <Label htmlFor={id} className="text-xs text-slate-400 font-medium ml-1">{label}</Label>
      {isEditing ? (
        <Select value={value} onValueChange={onChange}>
          <SelectTrigger id={id} className="w-full" aria-label={label}><SelectValue placeholder="Select..." /></SelectTrigger>
          <SelectContent>{options.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
        </Select>
      ) : <div className="px-3 py-2 bg-surface-800 rounded-lg text-slate-200 border border-transparent">{current || '—'}</div>}
    </div>
  )
}

export default function Campaign3LeadDetails() {
  ...
  const { prevId, nextId, goPrev, goNext, position, total, hasContext } = useLeadNavigator(id, listContext, {
    api: campaign3Api,
    listKey: 'campaign-3-leads',
    detailKey: 'campaign-3-lead',
    routeBase: '/campaigns/3',
  })
  ...
  const { data: lead, isLoading, error } = useQuery({
    queryKey: ['campaign-3-lead', id],
    queryFn: () => campaign3Api.getOne(id!).then(r => r.data),
    enabled: !!id,
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => campaign3Api.update(id!, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['campaign-3-lead', id] })
  ...
  const addNoteMutation = useMutation({
    mutationFn: (content: string) => campaign3Api.addNote(id!, content),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['campaign-3-lead', id] })
      setNewNote('')
  ...
  const setField = (name: string) => (value: string) => setFormData((prev: any) => ({ ...prev, [name]: value }))
  ...
  if (error || !lead) {
    return (
      <div className="text-center mt-10">
        ...
        <Button type="button" variant="secondary" className="mt-4" onClick={() => navigate('/campaigns/3')}>Go Back</Button>
      </div>
    )
  }
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button type="button" className="p-2 hover:bg-surface-700 text-slate-400 hover:text-slate-100 rounded-full transition-colors" onClick={() => navigate('/campaigns/3')}>
            <ArrowLeft size={20} />
          </button>
          <div>
            ...
            <p className="text-slate-400 text-sm">Campaign 3 · Added {formatRelativeTime(lead.createdAt)}</p>
          </div>
          {hasContext && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Campaign 3 Specific Details */}
          <div className="card">
            <h3 className="text-lg font-medium text-slate-100 mb-4 flex items-center gap-2">
              <FileText size={20} className="text-brand-500" /> Campaign Details
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SelectField id="preferredCountry" label="Preferred Country" value={formData.preferredCountry} current={lead.preferredCountry} options={COUNTRY_OPTIONS} isEditing={isEditing} onChange={setField('preferredCountry')} />
              <SelectField id="preferredIntake" label="Preferred Intake" value={formData.preferredIntake} current={lead.preferredIntake} options={INTAKE_OPTIONS} isEditing={isEditing} onChange={setField('preferredIntake')} />
              <SelectField id="currentQualification" label="Highest Qualification" value={formData.currentQualification} current={lead.currentQualification} options={QUALIFICATION_OPTIONS} isEditing={isEditing} onChange={setField('currentQualification')} />
              <SelectField id="course" label="Degree Name" value={formData.course} current={lead.course} options={DEGREE_OPTIONS} isEditing={isEditing} onChange={setField('course')} />
              <div className="space-y-1.5">
                <Label htmlFor="degreeField" className="text-xs text-slate-400 font-medium ml-1">Degree Field</Label>
                {isEditing ? <Input id="degreeField" name="degreeField" value={formData.degreeField} onChange={handleChange} className="w-full" />
                  : <div className="px-3 py-2 bg-surface-800 rounded-lg text-slate-200 border border-transparent">{lead.degreeField || '—'}</div>}
              </div>
              <SelectField id="completionYear" label="Completion Year" value={formData.completionYear} current={lead.completionYear} options={COMPLETION_YEAR_OPTIONS} isEditing={isEditing} onChange={setField('completionYear')} />
              <SelectField id="percentage" label="Percentage" value={formData.percentage} current={lead.percentage} options={PERCENTAGE_OPTIONS} isEditing={isEditing} onChange={setField('percentage')} />
              <SelectField id="mode" label="Consultation Mode" value={formData.mode} current={lead.mode} options={MODE_OPTIONS} isEditing={isEditing} onChange={setField('mode')} />
              <div className="space-y-1.5 md:col-span-2">
                <Label htmlFor="remarks" className="text-xs text-slate-400 font-medium ml-1">Detailed Remarks</Label>
